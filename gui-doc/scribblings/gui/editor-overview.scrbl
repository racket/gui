...
Most editor methods work in terms of @deftech{grapheme positions}
 instead of (character) @tech{positions}. Method names of
 ...
 back to a grapheme position as needed. All methods of @racket[snip%]
 work in terms of character positions, however, and a @racket[text%]
 object's internal state uses character positions. It is possible, for
 example, for the selection range in a @racket[text%] object to start
 or end in the middle of a grapheme. Also grapheme cluster that is
 within a single snip will render as a single grapheme, but a grapheme
 cluster that spans a snip boundary will be rendered as two partial
 graphemes

Event-handling methods such as inserting a character or moving the
 selection stay aligned to graphemes, and so a user should perceive an
 @tech{item} as a @tech{grapheme}. As character-based content is
