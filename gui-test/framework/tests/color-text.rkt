...
(define (random-test)
  ...
  (define (random-string)
    (build-string
     ...
     (λ (i)
       (case (random 3)
         [(0 1) (pick-a-char #\0 10)]
         [(2) (pick-a-char #\a 26)]))))

  (define (pick-a-position)
    (if (zero? (send t last-position))
        0
        (random (send t last-position))))
  
  (for ([i (in-range 10)])
    (define where (pick-a-position))
    (case (random 2)
      [(0)
       (send t insert (random-string) where where)]
      [(1)
       (define len (+ 1 (random 3)))
       (send t delete where len)]))
             
  (define first-result (get-colors t))
  
  (send t insert (send t get-text) 0 (send t last-position))
  (send t freeze-colorer)
  (send t thaw-colorer)
  
  (check-equal? (get-colors t) first-result))
