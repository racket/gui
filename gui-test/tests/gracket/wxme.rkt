...
(let ()
  ...
  (expect (send t char-to-grapheme-position 5) 4)
  (expect (send t grapheme-to-char-position 5) 6)
  (send t insert "a" 1)
  (expect (send t char-to-grapheme-position 0) 0)
  (expect (send t char-to-grapheme-position 1) 1)
  (expect (send t char-to-grapheme-position 2) 2)
  (expect (send t char-to-grapheme-position 5) 4)
  (expect (send t grapheme-to-char-position 5) 6)
  (expect (send t last-position/char) 7)
  (send t set-position/char 4)
  (send t delete)
  (expect (send t last-position/char) 5)
  (send t insert "e\u300")
  (expect (send t last-position/char) 7)
  (expect (send t grapheme-to-char-position 6) 7)
  (expect (send t char-to-grapheme-position 2) 2)
  (expect (send t char-to-grapheme-position 3) 2)
  (expect (send t char-to-grapheme-position 4) 3)
  (expect (send t char-to-grapheme-position 7) 6)
  (send t insert "\n" 1)
  (expect (send t last-position/char) 8)
  (expect (send t char-to-grapheme-position 8) 7)
  (expect (send t char-to-grapheme-position 3) 3)
  (expect (send t char-to-grapheme-position 5) 4))
    ...
    (apply string-append
           (map (lambda (l) (string-append l "\n"))
                '("#lang racket"
                  ...
                  "#false"))))
  (define (check-prog t)
    (expect (send t get-text 0 (send t last-position))
            prog)
    (let loop ([snip (send t find-first-snip)])
      (when snip
        ...
        (loop (send snip next))))
    (expect (send t last-position/char) (string-length prog))
    (expect (send t last-position) (string-grapheme-count prog))
    (let ([counts (make-vector (add1 (string-length prog)) 0)])
      ...
      (for ([i (in-range (string-length prog))])
        (unless (= (send t char-to-grapheme-position i) (vector-ref counts i))
          (printf "wrong at ~a: ~a ~a\n" i (send t char-to-grapheme-position i) (vector-ref counts i))))))
  ;; whole string
  (let ()
    (define t (new text%))
    (send t insert prog)
  ...
  (let ()
    ...
    (let loop ([i 0])
      (unless (= i (string-length prog))
        ...
        (define end (+ i len))
        (send t insert/char (substring prog i end) (send t get-end-position/char) 'same #f #t) ; <- `#t` here matters
        (define start (send t grapheme-to-char-position (send t char-to-grapheme-position i)))
        (send t change-style/char bold start end)
        (send t change-style/char normal end end)
        (loop (+ i len))))
    (check-prog t))
...
(let ()
  ...
  (expect (for/list ([i (in-range (send t last-position/char))])
            (send t char-to-grapheme-position i))
          '(0 1 2 3 4 5 6 6 7 8 9 10 11 12 13))
  (expect (for/list ([i (in-range (send t last-position/char))])
            (send t grapheme-to-char-position i))
