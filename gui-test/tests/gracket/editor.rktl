...
(define (run-save/load-tests editor% insert reset?)
  ...
  (let* ([mode #f]
	 ...
	 [ck-mode (lambda (-mode)
         ...
         [ck-type/creator (λ (path _expected-creator _expected-type)
                            (define-values (expected-creator expected-type)
                              (if (equal? (system-type) 'macosx)
                                  (values _expected-creator _expected-type)
                                  (values #"????" #"????")))
                            (define-values (got-creator got-type) (file-creator-and-type path))
                            (test expected-creator 'creator got-creator)
                            (test expected-type 'type got-type))])
    (insert e "Hello")
    (st #t e is-modified?)
    (st #f e get-filename)
    (when (eq? editor% text%)
    ...
    (st #f e is-modified?)
    (ck-text "tmp99")
    (ck-type/creator "tmp99" #"mReD" (if (eq? editor% text%) #"TEXT" #"WXME"))
    (insert e "Ack")
    (st #t e is-modified?)
    ...
    (let ([now (file-or-directory-modify-seconds "tmp99")])
      (send e set-file-creator-and-type #"XYZW" #"ABCD")
      (st #t e save-file "tmp99" 'same)
      (ck-text "tmp99")
      (ck-type/creator "tmp99" #"XYZW" #"ABCD")
      (st (string->path "tmp99") e get-filename)
