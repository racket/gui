    ...
    (mixin ((class->interface text%)) (indent-guides<%>)
      ...
      (define/augment (after-insert start len)
        (when guides
          ...
          (define para1 (scan-blank-backwards (position-paragraph start)))
          (define para2 (position-paragraph (+ start len)))
          (skip-list-expand! guides para1 para2)
          (recalculate-lines-guides para1 para2))
      ...
      (define/augment (on-delete start len)
        (when guides
          (begin-edit-sequence)
          (set! on-delete-contract-start (scan-blank-backwards (position-paragraph start)))
      ...
      (define/augment (after-delete start len)
        (when guides
          ...
          (recalculate-lines-guides on-delete-contract-start on-delete-contract-start)
          (end-edit-sequence))
